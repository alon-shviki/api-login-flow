...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      // Save token to DB
      await conn.execute("UPDATE users SET token = ? WHERE id = ?", [
        token,
        rows[0].id,
      ]);
      ...
      // CHANGED: Added failed login event to Kafka and structured log
      // REASON: Now we monitor both successful and failed login attempts
      const failedLogData = {
        timestamp: new Date().toISOString(),
        userId: null,
      ...
      logger.info(JSON.stringify(failedLogData));

      await producer.send({
        topic: "user-logins",
        messages: [{ value: JSON.stringify(failedLogData) }],
      });
...
app.get("/api/profile", async (req, res) => {
  ...
  try {
    ...
    if (users.length > 0) {
      res.send(
        `SUCCESS: Welcome ${users[0].email}. Your token is valid in TiDB.`
      );
    } else {
